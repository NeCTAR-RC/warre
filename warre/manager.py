...
class Manager:
    ...
    def create_reservation(self, context, reservation):
        ...
        if (
            not flavor.is_public
            and flavor.projects.filter_by(
        ...
        if reservation.total_hours > flavor.max_length_hours:
            raise exceptions.InvalidReservation(
                "Reservation is too long, max allowed is "
                f"{flavor.max_length_hours} hours"
            )

        if flavor.start and flavor.start > reservation.start:
            raise exceptions.InvalidReservation(
                "Reservation start time before flavor start time of "
                f"{flavor.start}"
            )
    ...
    def extend_reservation(self, context, reservation, new_end):
        ...
        if (new_end - now).total_seconds() > (
            ...
            raise exceptions.InvalidReservation(
                "Reservation is too long, max allowed is "
                f"{reservation.flavor.max_length_hours} hours"
            )

        free_slots = self.flavor_free_slots(
            context, reservation.flavor, reservation.end, new_end, reservation
        )
    ...
    def flavor_free_slots(self, context, flavor, start, end, reservation=None):
        ...
        for point, kind in time_list:
            if last_point is not None:
                segments.append(
                    {"start": last_point, "end": point, "slot": current_slot}
                )
            # update last_point
            last_point = point
            # adjust current slot
            current_slot = (
                current_slot + 1 if kind == "start" else current_slot - 1
            )
        # Pass2: only keep slot >= maximum capacity, a.k.a. busy slots
        busy_slots = [s for s in segments if s["slot"] >= flavor.slots]
                    ...
                    free_slots.append(
                        {
                            "start": start_free,
                            "end": (
                                s["start"] - datetime.timedelta(seconds=1)
                            ).replace(second=0),
                        }
                    )
                start_free = s["end"]
        ...
        if start_free < end:
            if start_free != start:
                start_free += datetime.timedelta(seconds=60)
            free_slots.append({"start": start_free, "end": end})
